...
export default function Albums() {
  ...
  return (
    <>
      ...
      {/* Lightbox Modal with Slideshow & Touch Swipe Support */}
      <AnimatePresence>
        {lightbox && currentAlbum && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-100 flex items-center justify-center bg-black/94 p-4 sm:p-8 backdrop-blur-md select-none"
            onClick={() => {
              setLightbox(null);
              setIsSlideshow(false);
            }}
          ...
          >
            ...
            <div className="absolute top-4 right-4 sm:top-8 sm:right-8 flex items-center gap-2 z-10">
              ...
              >
                {isSlideshow ? <Pause className="w-5 h-5" /> : <Play className="w-5 h-5 fill-current" />}
              </button>

              {/* Download image button */}
              <a
                href={`/assets/${currentAlbum.folder}/${currentAlbum.images[lightbox.imageIndex]}`}
                download
                onClick={(e) => e.stopPropagation()}
                className="p-2.5 text-white/70 hover:text-white bg-white/10 hover:bg-white/20 rounded-full backdrop-blur-md transition-all cursor-pointer"
            ...
            >
              ...
              <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 text-white/80 text-xs sm:text-sm tracking-wide bg-white/10 px-4 py-1.5 rounded-full backdrop-blur-md flex items-center gap-2">
                ...
                {isSlideshow && <span className="text-pink-300 font-medium">· Reproduciendo</span>}
